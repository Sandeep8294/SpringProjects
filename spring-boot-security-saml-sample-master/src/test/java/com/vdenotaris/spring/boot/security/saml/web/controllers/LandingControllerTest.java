 ...
 * org.springframework.web.method.support.ModelAndViewContainer; import
 ...
 * 
 * @WebAppConfiguration public class LandingControllerTest extends
 * CommonTestSupport {
 * 
 * 
 ...
 * standaloneSetup(landingController) .setCustomArgumentResolvers(new
 * MockArgumentResolver()) .setSingleView(mockView).build(); }
 * 
 * @Test public void testAnonymousLanding() throws Exception {
 * mockMvc.perform(get("/landing").session(mockHttpSession(true)))
 * .andExpect(status().isOk()) .andExpect(model().attribute("username",
 * USER_NAME)) .andExpect(view().name("pages/landing")); }
 * 
 * private static class MockArgumentResolver implements
 ...
 * ModelAndViewContainer modelAndViewContainer, NativeWebRequest
